        ...
        <div v-for="(item,index) in questionInfo" :key="index" style="margin-top: 15px">
          <div>
            ...
            <span style="color: red;font-style: italic;font-weight: 400;">&nbsp;
              {{ questionScore.get(item.questionId) }}分</span>
          </div>
          <!--题目中的配图-->
          ...
          <div style="margin-top: 25px" v-show="item.questionType === 4">
            ...
            <div class="ques-analysis">
              ...
              <div>
                我的回答：
                <span style="color: orange" v-for="(i2,index2) in userAnswer[index].split(';')">
                  {{ index2 + 1+ '、' + i2+"&nbsp;&nbsp;&nbsp;&nbsp;" }}
                </span>
              </div>
            </div>
          ...
          <!--简答题的答案-->
          <div style="margin-top: 25px" v-if="item.questionType === 5">
            <div class="ques-analysis">
              <h3 style="font-weight: 400">我的回答：</h3>
              <p style="font-weight: 400;color: orange"> {{ userAnswer[index] }} </p>
            </div>
            <span>评分：</span>
            <el-input-number v-model="scores[index]" :min="0"
                             :max="questionScore.get(item.questionId)"></el-input-number>
          </div>
      ...
      await exam.getExamQuestionByExamId(examId).then((resp) => {
        if (resp.code === 200) {
          ...
          resp.data.questionIds.split(',').forEach((item, index) => {
            this.questionScore.set(parseInt(item), parseInt(scores[index]))
          })
        }
      })
    },
      ...
      this.questionInfo.forEach((item,index) => {
        if (item.questionType === 5) {
          if (this.scores[index] == null) {
            this.$message.error("请先给第"+(index+1)+"题打分")
            isDf =  false
          }
          otherScore += this.scores[index]
        }
      })
      if (!isDf) return false
